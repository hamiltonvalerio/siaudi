...
class GiixModelCode extends ModelCode {
    ...
    public function prepare() {
        ...
        foreach ($tables as $table) {
            ...
            if (key_exists($className, $this->relations) && $this->relations[$className] && is_array($this->relations[$className])) {

                foreach ($this->relations[$className] as $relationName => $relationData) {
                    $temp = explode(',', $relationData);
                    ...
                    if (strpos('.', $temp[1]) === false) {
                        ...
                        if(!empty ($temp2[1])){
                            $temp2[1][0] = strtoupper($temp2[1][0]);
                            $classe = $temp2[1];
                        }else{
                            $temp2[0][0] = strtoupper($temp2[0][0]);
                            $classe = $temp2[0];
                        }

                        $arrayStr = implode(',', $temp);
                        $this->relations[$className][$relationName] = $arrayStr;
                    }
                }
            }
            $params = array(
