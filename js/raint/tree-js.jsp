...
// cabecalho include file=/taglib-imports.jspf
	...
	$().ready(function() {
		...
		$('ul.treeview_template').tree({
			checkbox: false,
                        ...
                        url: "<? echo $baseUrl . "/Raint/CarregaRaintAjax" . "?exercicio=" . $_GET["exercicio"];?>",
	...
	function ordenaNodes(pTree, nodes, nodePai){
        ...
        function envia_raint_json(sair){
            if (sair==1) { $('#raint_sair').val("1"); }
            if (sair==2) { $('#raint_sair').val("2"); }
            
            confirmarDados();
            
            $('#raint').val(getItensJson());
            $('#raint-form').submit();
            $('#raint_sair').val("0");
        }
