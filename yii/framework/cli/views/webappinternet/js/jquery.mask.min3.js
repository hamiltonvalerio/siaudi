...
(function(b) {
    var u = function(a, g, e) {
        ...
        c.init = function() {
            ...
            c.byPassKeys = [];
            c.translation = {0: {pattern: /\d/}, 9: {pattern: /\d/, optional: !0}, A: {pattern: /[a-zA-Z0-9]/}, S: {pattern: /[a-zA-Z]/}};
        ...
        var f = {events: function() {
                ...
                a = a || window.event;
                if (-1 === b.inArray(a.keyCode || a.which, c.byPassKeys))
                    return f.val(f.getMasked()),
                            f.callbacks(a)
            }, getMasked: function() {
                var a = [], b = f.val(), d = 0, t = g.length, h = 0, l = b.length, k = 1, m = "push", n;
                ...
                for (; n(); ) {
                    var p = g.charAt(d), q = c.translation[p], r = b.charAt(h);
                    q ? (r.match(q.pattern) ? (a[m](r), d += k) : !0 == q.optional && (d += k, h -= k), h += k) : (a[m](p), r == p && (h += k), d += k)
                }
                return a.join("")
            }, callbacks: function(b) {
                var c = f.val(), d = f.val() !== l;
                if (!0 === d && "function" == typeof e.onChange)
                    e.onChange(c, b, a, e);
                if (!0 ===
                        d && "function" == typeof e.onKeyPress)
                    e.onKeyPress(c, b, a, e);
                if ("function" === typeof e.onComplete && c.length === g.length)
                    e.onComplete(c, b, a, e)
            }};
        c.remove = function() {
            f.destroyEvents();
        ...
        c.getCleanVal = function() {
            for (var a = [], b = f.val(), d = 0, e = g.length; d < e; d++)
                c.translation[g.charAt(d)] && a.push(b.charAt(d));
            return a.join("")
        };
        c.init()
    };
